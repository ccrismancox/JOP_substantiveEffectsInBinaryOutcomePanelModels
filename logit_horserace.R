...
for(i in 1:nrow(NT)){
  ...
  out <- foreach(b= 1:B, .combine = 'rbind',
                 ...
                 .errorhandling = "remove") %dorng%{
                   ...
                   # t7 <- system.time({g7 <- try(geeglm(y~X+(state)-1, family=binomial, data=data2, x=T, id=state,
                   # corstr = "exchangeable"))})[3]
                   ...
                   if(class(g2)=="try-error"||!g2$conv||any(is.na(g2$coef))|| abs(g2$coef[1]) > M){
                     ...
                     # prob.g2.new <- predict(g2, newdata=newData2, type="response")
                   ...
                   if(class(g3)=="try-error"||!(g3$conv==0) || any(is.na(g3$coefficients))||any(abs(g3$coefficients) > M)){
                     ...
                     # prob.g3.new <- plogis(drop(X.mem %*% g3$coef))
                   ...
                   if(class(g5)=="try-error"||g5@optinfo$conv$opt!=0||anyNA(coef(g5)$state)||abs(unique(coef(g5)$state$X)) > M){
                     ...
                     # mar5 <-abs(mean( unique(coef(g5)$state$X)* dlogis(rowSums(cbind(1,X, Z) * as.matrix(coef(g5)$state) %x% rep(1, T)))-
                     #                  -2*dlogis(-2*X + c*gamma)))
                     if(any(abs(c(beta.CRE, alpha.CRE))> M)){
                       ...
                       # ame5 <- ((predict(g5, type="response", newdata=newData)*(1-predict(g5, type="response", newdata=newData))*(coef(g5)$state$X[1])))
                     }
                   }
                   if(class(g6)=="try-error"||g6@optinfo$conv$opt!=0||anyNA(coef(g6)$state)||abs(unique(coef(g6)$state$X)) > M){
                     ...
                     if(any(abs(c(beta.RE, alpha.RE))> M)){
                       ...
                       # ame6 <- ((predict(g6, type="response", newdata=newData)*(1-predict(g6, type="response", newdata=newData))*(coef(g6)$state$X[1])))
                     }
                   }
                   ...
                   if(class(g.beck)=="try-error"|| any(is.na(g.beck$coef))||any(abs(g.beck$coef) > M)){
                     ...
                     # p.beck.new <-  plogis(drop(X.mem2 %*% g.beck$coefficients))
                   ...
                   # m3 <- mean((gamma*alpha[keep]-g3$coef[-c(1, length(g3$coef))])^2)
                   ...
                   # m7 <- mean((gamma*alpha-alpha.RE)^2)
                   
                   
                   # b3 <-(mean(gamma*alpha[keep]-g3$coef[-c(1, length(g3$coef))]))
                   ...
                   # ame.truth <- (plogis(-2*newData$X + alpha*gamma)*(1-plogis(-2*newData$X + alpha*gamma))*-2)
                   ...
                   output <- c(g4$coef[1],
                               ...
                               # bias.ame2,  bias.ame3, bias.ame5, bias.ame6, bias.ame.beck,
                               # mse.ame2,  mse.ame3, mse.ame5, mse.ame6, mse.ame.beck,
  ...
  colnames(out) <- c("CML", "MLDV",  "FFE", "CRE", "RE", "Beck",
                     ...
                     # "mem.MLDV", "mem.FFE", "mem.CRE", "mem.RE",
                     # "bias.mem.MLDV", "bias.mem.FFE","bias.mem.CRE","bias.mem.RE", "bias.mem.beck",
                     # "mse.mem.MLDV", "mse.mem.FFE","mse.mem.CRE","mse.mem.RE", "mse.mem.beck",
  ...
  results[[i]] <- out
  cat("Results for t=", T, "n=", N,"\n")
  print(summary(out))
}
stopCluster(cl)


save.image("MCset_2July2019.rdata")


# 
rm(list=ls())
load("MCset_2July2019.rdata")
library(ggplot2)
library(extrafont)
library(matrixStats)
library(reshape2) 
...
p5data <- cbind(NT, do.call(rbind,
                            lapply(results,
                                   function(x){ colMeans(x[,c( #"bias.firth", "bias.cre",
...
#PML/CRE = about 7.5 (31.3)
#MLDV/PML = about 1.5 (1.3)
#MLDV/CRE = about 10.7 (43.8)
...
#PML/CRE = about 2.9 (5.4)
#MLDV/PML = about 3.5 (4.4)
#MLDV/CRE = about 6.7 (34.8)
...
#PML/CRE = about 1.5 (1.5)
#MLDV/PML = about 11.1 (12.8)
#MLDV/CRE = about 19.1 (17.7)
...
g1 <- ggplot(p1data[! p1data$Estimator %in% c("RE","Beck", "LPM", "PML"),])+ #RMSE in beta
  ...
  facet_grid(Rare~T )+
  # geom_text(aes(x=95,y=Inf, label=paste("atop(bar(y)==", ybar, ",", "textstyle('%'~dropped) == ", dropped, ")", sep="")),  
  ...
  ylab('RMSE')+
  xlab(expression(N))+
  # ggtitle(expression('RMSE in panel logit estimates of'~beta))+
...
g2 <- ggplot(p2data[! p2data$Estimator %in% c("RE","Beck", "LPM", "PML"),])+ #bias in beta
  ...
  ylab(' Absolute Bias')+
  xlab(expression(N))+
  # ggtitle(expression("Bias in"~ beta~ "Estimates"))+
  scale_linetype_manual(values=c("dotted", "solid", "dashed", "dotdash"))+
  scale_color_manual(values=hue_pal()(4)[c(2,1,3,4)])+
  theme(text=element_text(family="CM Roman"),
        legend.position="bottom",
...
g3 <- ggplot(p3data[! p3data$Estimator %in% c("RE","Beck", "LPM", "PML"),])+ #rmse in c
  ...
  ylab('RMSE')+
  xlab(expression(N))+
  # ggtitle(expression('RMSE in panel logit estimatates of'~italic(c[i])))+
...
g4 <- ggplot(p4data[!p4data$Estimator %in% c("RE","Beck", "LPM", "PML"),])+ #bias in c
  ...
  ylab('Absolute Bias')+
  xlab(expression(N))+
  # ggtitle(expression("Bias in"~ italic(c[i])~ "Estimates"))+
  scale_linetype_manual(values=c( "solid", "dashed", "dotdash"))+
  scale_color_manual(values=hue_pal()(4)[c(1,3,4)])+
  theme(text=element_text(family="CM Roman"),
        legend.position="bottom",
...
g9 <- ggplot(p9data[! p9data$Estimator %in% c("RE","Beck", "LPM", "PML"),])+
  geom_line(aes(x=N, y=(Time), color=Estimator, linetype=Estimator), size=1)+
  facet_grid(Rare~T )+
  # geom_text(aes(x=95,y=Inf, label=paste("atop(bar(y)==", ybar, ",", "textstyle('%'~dropped) == ", dropped, ")", sep="")),  
  #           data=adInfo, parse=TRUE, vjust=1.1, size=5, hjust=.9, check_overlap = TRUE) +
  geom_text(aes(x=95,y=Inf, label=label),
...
h=7; w=9
pdf(file="/home/cox/Dropbox/xtlogit/figures/betaRMSE_2Jul2019.pdf", height=h, width=w, family="CM Roman"); g1; dev.off()
pdf(file="/home/cox/Dropbox/xtlogit/figures/betabias_2Jul2019.pdf", height=h, width=w, family="CM Roman"); g2; dev.off()
pdf(file="/home/cox/Dropbox/xtlogit/figures/cRMSE_2Jul2019.pdf", height=h, width=w, family="CM Roman"); g3; dev.off()
pdf(file="/home/cox/Dropbox/xtlogit/figures/cbias_2Jul2019.pdf", height=h, width=w, family="CM Roman"); g4; dev.off()
pdf(file="/home/cox/Dropbox/xtlogit/figures/pred_2Jul2019.pdf", height=h, width=w, family="CM Roman"); g5; dev.off()
pdf(file="/home/cox/Dropbox/xtlogit/figures/predBias_2Jul2019.pdf", height=h, width=w, family="CM Roman"); g6; dev.off()
pdf(file="/home/cox/Dropbox/xtlogit/figures/ameBias_2Jul2019.pdf", height=h, width=w, family="CM Roman"); g7; dev.off()
pdf(file="/home/cox/Dropbox/xtlogit/figures/ameRMSE_2Jul2019.pdf", height=h, width=w, family="CM Roman"); g8; dev.off()

embed_fonts(file="/home/cox/Dropbox/xtlogit/figures/betaRMSE_2Jul2019.pdf")
embed_fonts(file="/home/cox/Dropbox/xtlogit/figures/betabias_2Jul2019.pdf")
embed_fonts(file="/home/cox/Dropbox/xtlogit/figures/cRMSE_2Jul2019.pdf")
embed_fonts(file="/home/cox/Dropbox/xtlogit/figures/cbias_2Jul2019.pdf")
embed_fonts(file="/home/cox/Dropbox/xtlogit/figures/pred_2Jul2019.pdf")
embed_fonts(file="/home/cox/Dropbox/xtlogit/figures/predBias_2Jul2019.pdf")
embed_fonts(file="/home/cox/Dropbox/xtlogit/figures/ameBias_2Jul2019.pdf")
embed_fonts(file="/home/cox/Dropbox/xtlogit/figures/ameRMSE_2Jul2019.pdf")
...
final.table <- rbind(c(p2data[p2data$N==100 & p2data$T== "T = 25" & p2data$Rare=="Non-rare Event",]$Bias, NA),
                     c(p1data[p1data$N==100 & p1data$T== "T = 25" & p1data$Rare=="Non-rare Event",]$RMSE, NA),
                     c(NA,p6data[p6data$N==100 & p6data$T== "T = 25" & p6data$Rare=="Non-rare Event",]$Bias),
                     c(NA,sqrt(p5data[p5data$N==100 & p5data$T== "T = 25" & p5data$Rare=="Non-rare Event",]$MSE)),
                     c(NA,p7data[p7data$N==100 & p7data$T== "T = 25" & p7data$Rare=="Non-rare Event",]$Bias),
...
rownames(final.table) <- c("Bias in $\\hat\\beta$", 
                           "RMSE in $\\hat\\beta$", 
                           "Bias in $\\hat{p}$", "RMSE in $\\hat{p}$",
                           "Bias in AME", "RMSE in AME")

(final.table/final.table[,"CRE"])[,-3]

library(xtable)
print(xtable((final.table/final.table[,"CRE"])[,-3], digits=2), sanitize.text.function=function(x){x})





final.table.rare <- rbind(c(p2data[p2data$N==100 & p2data$T== "T = 25" & p2data$Rare=="Rare event",]$Bias, NA),
                          c(p1data[p1data$N==100 & p1data$T== "T = 25" & p1data$Rare=="Rare event",]$RMSE, NA),
                          c(NA,p6data[p6data$N==100 & p6data$T== "T = 25" & p6data$Rare=="Rare event",]$Bias),
                          c(NA,sqrt(p5data[p5data$N==100 & p5data$T== "T = 25" & p5data$Rare=="Rare event",]$MSE)),
                          c(NA,p7data[p7data$N==100 & p7data$T== "T = 25" & p7data$Rare=="Rare event",]$Bias),
...
rownames(final.table.rare) <- c("Bias in $\\hat\\beta$", 
                                "RMSE in $\\hat\\beta$", 
                                "Bias in $\\hat{p}$", "RMSE in $\\hat{p}$",
                                "Bias in AME", "RMSE in AME")


(final.table.rare/final.table.rare[,"CRE"])[,-3]

print(xtable((final.table.rare/final.table.rare[,"CRE"])[,-3], digits=2), sanitize.text.function=function(x){x})



summ <- cbind(NT,do.call(rbind,lapply(results, function(x){colMeans(x[,c("Prop.1", "Prop.dropped")])})))
mean(summ[summ$Var3==g[1],]$Prop.1)
mean(summ[summ$Var3==g[2],]$Prop.1)
mean(summ[summ$Var3==g[1],]$Prop.dropped)
# 
# # Presentation
# h=6; w=9
# 
# g1 <- g1 + theme_bw(20)+  theme(text=element_text(family="CM Sans"),
#                                      legend.position="bottom",
#                                      legend.title = element_text(size=18,
#                                                                  face="bold"),
#                                      legend.text = element_text(size = 18),
#                                      strip.text.y = element_text(size=14),
#                                      legend.key.size = unit(.5,"in"))
# g2 <- g2 + theme_bw(20)+  theme(text=element_text(family="CM Sans"),
#                                      legend.position="bottom",
#                                      legend.title = element_text(size=18,
#                                                                  face="bold"),
#                                      legend.text = element_text(size = 18),
#                                      strip.text.y = element_text(size=14),
#                                      legend.key.size = unit(.5,"in"))
# g3 <- g3 + theme_bw(20)+ theme(text=element_text(family="CM Sans"),
#                                     legend.position="bottom",
#                                     legend.title = element_text(size=18,
#                                                                 face="bold"),
#                                     legend.text = element_text(size = 18),
#                                     strip.text.y = element_text(size=14),
#                                     legend.key.size = unit(.5,"in"))
# g4 <- g4 + theme_bw(20)+ theme(text=element_text(family="CM Sans"),
#                                     legend.position="bottom",
#                                     legend.title = element_text(size=18,
#                                                                 face="bold"),
#                                     legend.text = element_text(size = 18),
#                                     strip.text.y = element_text(size=14),
#                                     legend.key.size = unit(.5,"in"))
# g5 <- g5 + theme_bw(20)+  theme(text=element_text(family="CM Sans"),
#                                      legend.position="bottom",
#                                      legend.title = element_text(size=18,
#                                                                  face="bold"),
#                                      legend.text = element_text(size = 18),
#                                      strip.text.y = element_text(size=14),
#                                      legend.key.size = unit(.5,"in"))
# g6 <- g6 + theme_bw(20)+ theme(text=element_text(family="CM Sans"),
#                                     legend.position="bottom",
#                                     legend.title = element_text(size=18,
#                                                                 face="bold"),
#                                     legend.text = element_text(size = 18),
#                                     strip.text.y = element_text(size=14),
#                                     legend.key.size = unit(.5,"in"))
# g7 <- g7 + theme_bw(20)+  theme(text=element_text(family="CM Sans"),
#                                      legend.position="bottom",
#                                      legend.title = element_text(size=18,
#                                                                  face="bold"),
#                                      legend.text = element_text(size = 18),
#                                      strip.text.y = element_text(size=14),
#                                      legend.key.size = unit(.5,"in"))
# g8 <- g8 + theme_bw(20)+  theme(text=element_text(family="CM Sans"),
#                                      legend.position="bottom",
#                                      legend.title = element_text(size=18,
#                                                                  face="bold"),
#                                      legend.text = element_text(size = 18),
...
#       
# 
# ggsave(g1, file="/home/cox/Dropbox/xtlogit/figures/betaRMSE_8Mar2019_pres.pdf", height=h, width=w)
# ggsave(g2, file="/home/cox/Dropbox/xtlogit/figures/betabias_8Mar2019_pres.pdf", height=h, width=w)
# ggsave(g3, file="/home/cox/Dropbox/xtlogit/figures/cRMSE_8Mar2019_pres.pdf", height=h, width=w)
# ggsave(g4, file="/home/cox/Dropbox/xtlogit/figures/cbias_8Mar2019_pres.pdf", height=h, width=w)
# ggsave(g5, file="/home/cox/Dropbox/xtlogit/figures/pred_8Mar2019_pres.pdf", height=h, width=w)
# ggsave(g6, file="/home/cox/Dropbox/xtlogit/figures/predBias_8Mar2019_pres.pdf", height=h, width=w)
# ggsave(g7, file="/home/cox/Dropbox/xtlogit/figures/ameBias_8Mar2019_pres.pdf", height=h, width=w)
# ggsave(g8, file="/home/cox/Dropbox/xtlogit/figures/ameRMSE_8Mar2019_pres.pdf", height=h, width=w)
# 
# embedFonts(file="/home/cox/Dropbox/xtlogit/figures/betaRMSE_8Mar2019_pres.pdf")
# embedFonts(file="/home/cox/Dropbox/xtlogit/figures/betabias_8Mar2019_pres.pdf")
# embedFonts(file="/home/cox/Dropbox/xtlogit/figures/cRMSE_8Mar2019_pres.pdf")
# embedFonts(file="/home/cox/Dropbox/xtlogit/figures/cbias_8Mar2019_pres.pdf")
# embedFonts(file="/home/cox/Dropbox/xtlogit/figures/pred_8Mar2019_pres.pdf")
# embedFonts(file="/home/cox/Dropbox/xtlogit/figures/predBias_8Mar2019_pres.pdf")
# embedFonts(file="/home/cox/Dropbox/xtlogit/figures/ameBias_8Mar2019_pres.pdf")
# embedFonts(file="/home/cox/Dropbox/xtlogit/figures/ameRMSE_8Mar2019_pres.pdf")
...
#                                    "CML", "FFE", "CRE")], na.rm=T) +
#                                      (colMeans(x[,c(
#                                        "CML", "FFE", "CRE")], na.rm=T)+ 2)^2)})))
# colnames(p1data)[colnames(p1data)=="FFE"] <- c("PML")
# 
# p2data <- cbind(NT, do.call(rbind, lapply(results,
#                                           function(x){ 
#                                             abs(colMeans(x[,c( "CML", "FFE", "CRE")]+2, na.rm=T))})))
# colnames(p2data)[colnames(p2data)=="FFE"] <- c("PML")
# 
# 
# 
...
#                                     "pred.FFE", "pred.CRE"
#                                   )], na.rm=T)})))
# colnames(p5data)[4:5] <- c("PML", "CRE")
# 
# 
# p6data <- cbind(NT, do.call(rbind,
#                             lapply(results,
#                                    function(x){ abs(colMeans(x[,c( #"bias.firth", "bias.cre",
#                                      "bias.pred.FFE", "bias.pred.CRE"
#                                    )], na.rm=T))})))
# colnames(p6data)[4:5] <- c("PML", "CRE")
# 
# p7data <- cbind(NT, do.call(rbind,
#                             lapply(results,
#                                    function(x){ abs(colMeans(x[,c( #"bias.firth", "bias.cre",
#                                      "bias.mem.FFE","bias.mem.CRE")],
#                                      na.rm=T))})))
# colnames(p7data)[4:5] <- c("PML", "CRE")
# 
# 
# p8data <- cbind(NT, do.call(rbind,
#                             lapply(results,
#                                    function(x){colMeans( (x[,c( #"bias.firth", "bias.cre",
#                                      "mse.mem.FFE","mse.mem.CRE")]), na.rm=T)})))
# colnames(p8data)[4:5] <- c("PML", "CRE")
# 
# biasMat <- merge(p2data, p4data, by=c("Var1", "Var2", "Var3"));
# biasMat <- merge(biasMat, p6data, by=c("Var1", "Var2", "Var3"))
# biasMat <- merge(biasMat, p7data, by=c("Var1", "Var2", "Var3"))